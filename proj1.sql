...
WHERE people.playerid = HallOfFame.playerid
  ...
  AND schools.schoolState = 'CA' -- GROUP BY collegeplaying.playerid
ORDER BY yearid DESC,
  schools.schoolid,
  people.playerid;
;
...
FROM people
  ...
  LEFT OUTER JOIN CollegePlaying ON HallOfFame.playerid = CollegePlaying.playerid
WHERE inducted = 'Y'
...
SELECT people.namefirst, people.namelast, (SUM(H-H2B-H3B-HR) + SUM(2*H2B) + SUM(3*H3B) + SUM(4*HR)+0.0) / SUM(AB) as slg
